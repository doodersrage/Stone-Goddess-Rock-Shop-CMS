...
recTmp.Open "SELECT * FROM NewItems WHERE recno = " & strstone, SomeDB
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" id="AutoNumber3" style="border-collapse: collapse">
    ...
    <tr>
      ...
      <td bgcolor="#FFFFFF"><div align="center"><div id="stalkdescr">Date Added: <strong>
        ...
        Name: <strong>
          <% Response.Write(recTmp("itemname")) %>
        </strong></div></div></td>
    ...
    <tr>
	  ...
	  <div id="stalkdescr"><% Response.Write(recTmp("items")) %></div>
      </td>
    </tr>
    <tr>
...
<% dim count1
	   ...
	   sql12 = "SELECT * FROM NewItems ORDER BY itemname ASC LIMIT " & strreccnt & ",25" 
  ...
  count1 = count1 + 1
%>
            <%=count1+strreccnt%>.<a style="text-decoration: none;" href="http://stonegoddess.com/Stone-Goddess-NewItems/?S=<%=recTmp2("recno")%>&amp;R=<%=strreccnt%>" ONMOUSEOVER="popup('<%=Left(recTmp2("items"),120)%>','lightgreen')"; ONMOUSEOUT="kill()"><strong> 
...
<% if strreccnt <> 0 then %>
<a href="http://stonegoddess.com/Stone-Goddess-NewItems/?S=<%=strstone%>&amp;R=<%=strreccnt-25%>" >Previous</a>
  <% end if %>
&nbsp;&nbsp;&nbsp;
<% if strreccnt = "" then %><a href="http://stonegoddess.com/Stone-Goddess-NewItems/?S=<%=strstone%>&amp;R=50">Next</a>
<%end if %>
<%if count1 = 25 and strreccnt <> "" then %><a href="http://stonegoddess.com/Stone-Goddess-NewItems/?S=<%=strstone%>&amp;R=<%=strreccnt+25%>">Next</a>
<% end if
end if%>
</div>
</div>
